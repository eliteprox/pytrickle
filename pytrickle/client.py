        ...
        try:
            while not self.stop_event.is_set() and not self.error_event.is_set():
                try:
                    ...
                    if frame is None:
                        # Check if this is due to queue being empty/timeout or actual sentinel
                        # If queue is empty but we haven't received stop signal, continue waiting
                        if not self.stop_event.is_set() and not self.error_event.is_set():
                            continue
                        else:
                            # Stop signal received, break out of processing loop
                            logger.info("Processing loop received stop signal, ending")
                            break
                    ...
                    if isinstance(frame, VideoFrame):
                        ...
                        if queue_size > self.max_queue_size * 0.6 or output_queue_size > self.max_queue_size * 0.6:
                            logger.warning(f"Queues filling up - input: {queue_size}/{self.max_queue_size}, output: {output_queue_size}/{self.max_queue_size}")
                        elif queue_size > self.max_queue_size * 0.3 or output_queue_size > self.max_queue_size * 0.3:
                            logger.info(f"Queue status - input: {queue_size}/{self.max_queue_size}, output: {output_queue_size}/{self.max_queue_size}")
                        else:
                            logger.debug(f"Queue status - input: {queue_size}/{self.max_queue_size}, output: {output_queue_size}/{self.max_queue_size}")
